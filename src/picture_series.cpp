...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    desc.add_options()
      ...
      ("picture_serie,ps", "Record Picture serie")
	  ("rate", po::value<float>(&rate), "Rate at which images are taken")
	  ("count", po::value<int>(&count), "The number of images that should be taken")
	  ("topics", po::value<std::vector<string> >(&image_topics), "image topics to record")
      ("snapshot,ss", "Take a snapshot")
	  ("rate", po::value<float>(&rate), "Rate at which images are taken")
	  ("count", po::value<int>(&count), "The number of images that should be taken")
	  ("topics", po::value<std::vector<string> >(&image_topics), "image topics to record");
